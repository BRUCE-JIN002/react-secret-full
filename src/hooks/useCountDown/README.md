...
function CountDownComponent() {
  ...
  return (
    <div>
      ...
      <div>
        {formattedTime.days}天 {formattedTime.hours}时{formattedTime.minites}分{" "}
...
function TargetCountDown() {
  ...
  return (
    <div>
      ...
      <div>
        {formattedTime.hours}:
        {formattedTime.minites.toString().padStart(2, "0")}:
...
function PreciseCountDown() {
  ...
  return (
    <div>
      精确倒计时: {formattedTime.seconds}.
      {Math.floor(formattedTime.minlliseconds / 100)}秒
    </div>
  );
}
```
...
1. **时间精度**: 默认 1 秒更新，可通过 `interval` 调整精度
2. **性能考虑**: 高频更新（如 100ms）会增加 CPU 使用率
