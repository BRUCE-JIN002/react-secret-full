...
function StyledComponent() {
  ...
  return <div className={className}>这是一个动态样式的组件</div>;
}
```
...
function ThemedComponent({ theme }) {
  ...
  return <button className={className}>{theme} 按钮</button>;
}
```
...
function ConditionalStyleComponent({ isActive, size }) {
  ...
  return <button className={className}>{isActive ? "激活" : "未激活"}</button>;
}
```
